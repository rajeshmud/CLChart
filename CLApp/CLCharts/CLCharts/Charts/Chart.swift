...
public class Chart {
    ...
    private let layers: [CLLayer]

    convenience public init(frame: CGRect, layers: [CLLayer]) {
        self.init(view: ChartBaseView(frame: frame), layers: layers)
    }
    
    public init(view: ChartBaseView, layers: [CLLayer]) {
    ...
    private func drawRect(rect: CGRect) {
        ...
        for layer in self.layers {
            layer.viewDrawing(context: context!, chart: self)
        }
    }
}
